...
if(mysqli_connect_errno()){
	...
	$sclname=$_GET['utd'];
	//echo $sclname;
	$query="select *  from req1718 where utd = '$sclname'";
...
<br/><br/>
		...
		<?php 
			$query1 = "select sum(req) from addreq where utd = '$sclname'";
		...
		<?php
				$query = "select * from addreq where utd = '$sclname'";
			$result = mysqli_query($connection,$query);
			$number_of_rows=mysqli_num_rows($result);
			if($number_of_rows>0){
...
<div id="myflash" class="flash" style="font-size:20px;padding-top:20px; min-height:100px; font-weight: bold;">
</div>
<script>

if(sum1==0&&sum2==0&&flag_add==1){
	
	document.getElementById("myflash").innerHTML="No Data uploaded!!";
}
</script>
